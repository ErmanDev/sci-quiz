...
interface CreateQuizScreenProps {
  onBack: () => void;
  onSelectQuestions: (config: { name: string; mode: GameMode; category: GameCategory; xp: number }) => void;
  onCreateQuizWithCards: (
    config: { name: string; mode: GameMode; category: 'Card' | 'Board'; xp: number },
    questions: Question[],
  ) => void;
  quizToEdit?: TeacherQuiz | null;
  onUpdateQuiz?: (updatedData: { name: string; mode: GameMode; category: GameCategory; xp: number }, cards: Question[]) => void;
  onAddCardToVault: (card: Question) => void;
}
...
const CreateQuizScreen: React.FC<CreateQuizScreenProps> = ({
  ...
  const handleEditorChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'points' || name === 'timeLimit' || name === 'xp') {
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (newType === 'multiple-choice') {
      ...
      setEditorCard({ ...baseData, type: 'identification', answer: '', points: 1, xp: 20 });
    }
  };
...
setFileName(file.name);
  ...
  const handleAddQuestionsFromVault = (selectedQuestions: Question[]) => {
    const newCards = selectedQuestions.map(q => {
      const newId = { ...q, id: Math.random() + Date.now() };
      // Set default points and XP based on question type, similar to the editor logic
      newId.points = 1; // Default points for any question from vault
      newId.xp = newId.type === 'multiple-choice' ? 10 : 20; // Default XP
      return newId;
    });
  ...
  const handleProceed = () => {
    if (quizToEdit && onUpdateQuiz) {
      onUpdateQuiz({ name: quizName, mode: gameMode, category: gameCategory, xp }, quizCards);
      return;
    }

    if (gameCategory === 'Card' || gameCategory === 'Board') {
      onCreateQuizWithCards({ name: quizName, mode: gameMode, category: gameCategory as 'Card' | 'Board', xp }, quizCards);
      return;
    }

    // Normal (create): keep previous flow trigger so parent opens Vault flow as you designed
    onSelectQuestions({ name: quizName, mode: gameMode, category: gameCategory, xp });
  };
    ...
    <div className="relative h-full flex flex-col text-white">
                ...
                <div className="grid grid-cols-12 gap-2">
                  ...
                  <div className="col-span-3">
                    ...
                    <input
                      ...
                      onChange={handleEditorChange}
                      className="w-full bg-brand-deep-purple/50 border border-brand-light-purple rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-brand-glow [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                  </div>
                  <div className="col-span-3">
      ...
      <footer className="absolute bottom-0 left-0 right-0 p-4 bg-brand-deep-purple/80 backdrop-blur-sm border-t border-brand-light-purple/30">
        <button
          ...
          className="w-full bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300 ease-in-out hover:bg-blue-500 hover:shadow-glow focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 disabled:bg-gray-500/50 disabled:cursor-not-allowed disabled:shadow-none disabled:text-gray-300"
        >
      ...
      {showComingSoon && (
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-black/70 text-white px-4 py-2 rounded-lg">
          COMING SOON!
        </div>
      )}
