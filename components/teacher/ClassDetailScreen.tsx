...
const ClassDetailScreen: React.FC<ClassDetailScreenProps> = ({ classData, onBack, students }) => {
  ...
  const startEdit = (a: Announcement) => {
    ...
    setEditBody(a.body);
    setEditDate(new Date(a.date).toISOString().split('T')[0]);
    setEditMsg('');
  };
    ...
    <div className="relative space-y-4 h-full flex flex-col">
      ...
      <div className="bg-brand-mid-purple/80 rounded-2xl p-4 text-white border border-brand-light-purple/50 shadow-lg flex-shrink-0">
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-xl font-bold">{t('announcements') || 'Announcements'}</h2>
          <div className="flex items-center gap-3">
            <input
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              className="bg-brand-deep-purple/50 border border-brand-light-purple/50 rounded-lg px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-glow"
              style={{width: '55px'}}
            />
            <button
              onClick={handleCreateAnnouncement}
              disabled={submitting || !title.trim() || !body.trim()}
              className="bg-blue-600 text-white font-semibold py-2 rounded-lg transition-all duration-300 hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:bg-gray-500/50 disabled:cursor-not-allowed"
              style={{width: '55px'}}
            >
