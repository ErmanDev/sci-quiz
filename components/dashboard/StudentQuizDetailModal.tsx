...
import { ClientQuizDone, ServerQuiz } from './QuizzesScreen'; // Adjusted import
import { Question, MultipleChoiceQuestion } from '../../data/teacherQuizQuestions';

interface StudentQuizDetailModalProps {
  isOpen: boolean;
  quiz: ClientQuizDone | null;
  serverQuiz: ServerQuiz | null; // Add serverQuiz prop
  onClose: () => void;
}
...
const StudentQuizDetailModal: React.FC<StudentQuizDetailModalProps> = ({ isOpen, quiz, serverQuiz, onClose }) => {
  const { t } = useTranslations();
  
  if (!isOpen || !quiz || !serverQuiz) {
    return null;
  }
  
  // Fallback for score if it's just a percentage
  const scoreDisplay = quiz.score.includes('/') 
    ? quiz.score 
    : `${quiz.score} (${serverQuiz.questions.length} questions)`;
    ...
    >
      ...
      >
        ...
        <div className="flex-shrink-0 text-center mb-4">
            <h2 className="text-2xl font-bold font-orbitron">{serverQuiz.title}</h2>
            <p className="text-brand-glow">{`${serverQuiz.type} - ${serverQuiz.mode}`}</p>
            <p className="text-4xl font-bold font-orbitron text-brand-glow mt-2">{scoreDisplay}</p>
        </div>
        ...
        <div className="flex-grow overflow-y-auto hide-scrollbar pr-2 space-y-3">
            ...
            {serverQuiz.questions && serverQuiz.questions.length > 0 ? (
                serverQuiz.questions.map((q, index) => {
                    // This is a simplified correctness logic. For real applications, you'd need student's answers.
                    const wasCorrect = index < parseInt(quiz.score.split('/')[0], 10);

                    return <QuestionDetail key={q.id || index} question={q as any} wasCorrect={wasCorrect} />;
                })
            ) : (
