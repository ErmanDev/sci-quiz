...
router.post('/', async (req, res) => {
  ...
  if (uIdx > -1) {
    ...
    if (Array.isArray(answers) && answers.length) {
      expGain = answers.reduce((sum: number, a: any) => {
        if (a?.wasCorrect) {
      ...
      // Fallback: if no answers, grant EXP based on percentage of score
      const quiz = (db.data!.quizzes || []).find((q: any) => String(q.id) === String(quizId));
      if (quiz && Array.isArray(quiz.questions)) {
        const totalQuizXp = quiz.questions.reduce((sum: number, q: any) => sum + Number(q.xp || 0), 0);
        if (percent > 0) {
          expGain = Math.round(totalQuizXp * (percent / 100));
        }
      }
    }
